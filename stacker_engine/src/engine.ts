...
export class Engine {
	...
	#handleInput(input: Input) {
		switch (input) {
			case "hold":
				...
				this.#setActive(Piece.spawn(newActive.type));
				break;
			case "rotateRight":
				this.#rotate(1);
				break;
	...
	#rotate(rotation: Rotation) {
		for (let i = 0; i < 5; i++) {
			...
			if (!this.#pile.checkCollision(branched.blocks)) {
	...
	#harddrop() {
		...
		this.#setActive(Piece.spawn(this.#generator.pull().type));
	...
	#fall() {
		...
		if (!this.#pile.checkCollision(branched.blocks)) {
			// Falling doesn't change the ghostPiece so we bypass #setActive() here.
			this.#activePiece = branched;
		}
	}
...
class Pile {
	...
	checkCollision(blocks: Coords[]): boolean {
	...
	calculateGhost(piece: Piece): Piece {
		while (true) {
			...
			if (this.checkCollision(branched.blocks)) {
				return piece;
			}
			piece = branched;
		}
